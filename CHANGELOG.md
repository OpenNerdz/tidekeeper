...
- HLS/DASH parsers resolve relative segment URLs and inherited DASH timelines; `{Duration}` is sanitized in Windows filenames.
...
- Starting a download applies current options without silently saving them or changing the TIDAL client.
- Album/artist progress stays below 100% until videos finish, and the Quality column can show the downloaded result.
